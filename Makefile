...
tarball:
	git archive --format=tar.gz --prefix=libjson-rpc-cpp-skale-$(GIT_VERSION)/ -o libjson-rpc-cpp-skale-$(GIT_VERSION).tar.gz HEAD

signed-taball: tarball
	gpg --armor --detach-sign libjson-rpc-cpp-skale-$(GIT_VERSION).tar.gz
